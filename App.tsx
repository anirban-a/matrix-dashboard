...
const App: React.FC = () => {
  ...
  const uniquePeaks = useMemo(() => {
    // Basic logic: group by mz, see if only 1 mineral exists in that range
    // For mock visualization, we'll just slice the data
    return filteredData.filter((p, i) => i % 5 === 0);
  }, [filteredData]);

  const commonPeaks = useMemo(() => {
    // Mocking common peaks (points where all selected minerals overlap)
      ...
      <aside className="w-64 border-r border-slate-800 bg-slate-900 flex flex-col shrink-0">
        <div className="p-6">
          ...
          <nav className="space-y-1 mb-8">
            <button className="flex items-center gap-3 w-full p-2.5 rounded-lg bg-cyan-500/10 text-cyan-400 font-medium transition-all">
